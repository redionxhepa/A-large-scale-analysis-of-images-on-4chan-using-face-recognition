...
for cluster in tqdm(clusters_dictionary):
         if cluster !="-1":            #"473":              #  if cluster != "-1":
            ...
            medoidEncoding=images_encodings[medoidIndex]
            resultv2 = compare_facesv2(encodingsWiki_list,medoidEncoding,tolerance)
            if resultv2 != -1:
               annotated_clusters[cluster]=wikiNames_list[resultv2]
...
MYDIR = 'Medoids_'+str(tolerance)
CHECK_FOLDER = os.path.isdir(MYDIR)
if not CHECK_FOLDER:
    pathlib.Path('Medoids_'+str(tolerance)).mkdir(parents=True, exist_ok=True)
...
with open('annotatedClusters_medoid_tolerance_'+str(tolerance)+'.txt','w') as file:
...
#to do list
#output a file  for each cluster with the medoid in it, name   annotation_id
#cluster_processed=0
